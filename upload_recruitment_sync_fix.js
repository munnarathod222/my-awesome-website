...
async function main() {
  console.log('Uploading Recruitment hybrid persistence and role filter updates...');
  await uploadFile(
    'apps/web/src/lib/recruitmentClient.js',
    'apps/web/src/lib/recruitmentClient.js',
    'fix: ensure hybrid persistent application storage between PocketBase and localStorage'
  );
  await uploadFile(
    'apps/web/src/pages/RecruitmentDashboardPage.jsx',
    'apps/web/src/pages/RecruitmentDashboardPage.jsx',
    'feat: add position role filter and display applicant role badge on recruitment portal'
  );

  console.log('\nUploading dist files...');
  await uploadFile('dist/apps/web/index.html', 'dist/apps/web/index.html', 'chore: update dist index.html for recruitment sync fix');
  console.log('Uploading JS bundle (4.9MB)...');
  await uploadFile('dist/apps/web/assets/index-B8U5_NfQ.js', 'dist/apps/web/assets/index-B8U5_NfQ.js', 'chore: update dist JS bundle');
