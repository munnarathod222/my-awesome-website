...
async function main() {
  console.log('Uploading Invoice PDF Layout & Due Date Fix...');
  await uploadFile(
    'apps/web/src/lib/downloadUtils.js',
    'apps/web/src/lib/downloadUtils.js',
    'fix: widen TOTAL AMOUNT DUE box, simplify bank header to BANK DETAILS, and remove currency text'
  );
  await uploadFile(
    'apps/web/src/pages/PaymentRequestsPage.jsx',
    'apps/web/src/pages/PaymentRequestsPage.jsx',
    'fix: pass selected payment request due date into invoice PDF due date'
  );

  console.log('\nUploading dist files...');
  await uploadFile('dist/apps/web/index.html', 'dist/apps/web/index.html', 'chore: update dist index.html for invoice PDF fix');
  console.log('Uploading JS bundle...');
  await uploadFile('dist/apps/web/assets/index-CblJ4dAP.js', 'dist/apps/web/assets/index-CblJ4dAP.js', 'chore: update dist JS bundle');

  console.log('\n✅ Invoice PDF Layout & Due Date Fix uploaded successfully!');
}
