...
function uploadFileToSupabase(localPath, remoteRelPath) {
  try {
...
function buildLocalPath(collectionId, recordId, filename) {
    ...
    try {
      ...
      collection.fields.forEach(field => {
        ...
        fileList.forEach(filename => {
          if (!filename) return;
          const localPath = buildLocalPath(collectionId, recordId, filename);
          const remotePath = `storage/${collectionId}/${recordId}/${filename}`;
          uploadFileToSupabase(localPath, remotePath);
        });
      });
    } catch (err) {
