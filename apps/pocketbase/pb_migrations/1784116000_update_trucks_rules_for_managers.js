...
migrate((app) => {
  ...
  try {
    const collection = app.findCollectionByNameOrId("trucks");
    const oldRolesRule = "@request.auth.role = 'admin' || @request.auth.role = 'dispatcher'";
    
    collection.createRule = oldRolesRule;
    collection.updateRule = oldRolesRule;
    collection.deleteRule = oldRolesRule;
