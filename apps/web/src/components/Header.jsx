...
export default function Header() {
  ...
  return (
    <>
      {/* Desktop Header & Unauthenticated Mobile Header */}
      <header className={cn(
      ...
      {/* Authenticated Mobile Top Action Bar */}
      {isAuthenticated && (
        <header className="md:hidden sticky top-0 z-50 w-full border-b border-white/5 bg-[#0C101B]/85 backdrop-blur-md px-4 py-3.5 flex items-center justify-between shadow-md">
          ...
          <div className="flex items-center gap-2.5">
            {/* Language Selector icon-only or simple text */}
            <div className="flex items-center bg-secondary/35 border border-white/5 rounded-full px-2 py-0.5">
              <Globe className="w-3.5 h-3.5 text-slate-400 shrink-0" />
              <Select value={language} onValueChange={setLanguage}>
                <SelectTrigger className="bg-transparent border-none shadow-none h-6 w-14 px-1 focus:ring-0 text-[10px] font-bold text-slate-300">
            ...
            <div className="w-7 h-7 rounded-full bg-slate-800 border border-white/10 flex items-center justify-center text-xs font-black text-primary">
              {(currentUser?.full_name || currentUser?.name || 'U').charAt(0).toUpperCase()}
            </div>
            
            {/* Logout button */}
            <Button variant="ghost" size="icon" onClick={handleLogout} className="w-8 h-8 text-slate-400 hover:text-destructive hover:bg-destructive/10 rounded-full transition-colors">
              <LogOut className="h-4 w-4" />
            </Button>
          </div>
        </header>
