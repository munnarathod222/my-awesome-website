...
const ExpensesPage = () => {
  ...
  const generateExpensesPDFBlob = () => {
    try {
      const doc = new jsPDF();
      const userName = pb.authStore.model?.name || 'Admin';
      
      // Header
      doc.setFontSize(20);
      doc.setTextColor(15, 23, 42);
      doc.text('Expenses Statement', 14, 22);
      ...
      doc.text(`Total Fleet Expenses: Rs. ${total.toFixed(2)}`, 20, 50);
      doc.setFont(undefined, 'normal');

      // Table
      const tableColumn = ["Date", "Category", "Description", "Vehicle", "Amount"];
      const tableRows = expenses.map(tx => [
      ...
      console.error('Failed to generate Expenses PDF:', e);
      return null;
    }
  };
  ...
  const triggerEmailExpenses = () => {
    ...
    const pdfBlob = generateExpensesPDFBlob();
    const fileName = `expenses_report_${format(new Date(), 'yyyyMMdd')}.pdf`;

    setMailData({ 
      recipient: '', 
      subject: 'Expenses Report – Jai Bhavani Cargo', 
      body: 'Hi, please find the attached expenses report document.', 
      html, 
