...
async function main() {
  console.log('Uploading Driver Trip Logs Fix...');
  await uploadFile(
    'apps/web/src/lib/driverUtils.js',
    'apps/web/src/lib/driverUtils.js',
    'fix: exclude terminated and absconded drivers from new trip selection dropdown'
  );
  await uploadFile(
    'apps/web/src/pages/TripLogsPage.jsx',
    'apps/web/src/pages/TripLogsPage.jsx',
    'fix: preserve driver name rendering on trip logs and fallback to Dayanand Surwase'
  );
  await uploadFile(
    'apps/web/src/components/AddTripModal.jsx',
    'apps/web/src/components/AddTripModal.jsx',
    'fix: include Dayanand Surwase option and filter terminated/absconded drivers'
  );
  await uploadFile(
    'apps/web/src/components/TripEditModal.jsx',
    'apps/web/src/components/TripEditModal.jsx',
    'fix: preserve historical driver name in edit modal'
  );

  console.log('\nUploading dist files...');
  await uploadFile('dist/apps/web/index.html', 'dist/apps/web/index.html', 'chore: update dist index.html for driver trip fix');
  console.log('Uploading JS bundle...');
  await uploadFile('dist/apps/web/assets/index-BAtToUzi.js', 'dist/apps/web/assets/index-BAtToUzi.js', 'chore: update dist JS bundle');

  console.log('\n✅ Driver Trip Logs Fix uploaded successfully!');
}
