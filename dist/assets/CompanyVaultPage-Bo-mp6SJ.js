import{r as o,R as es,j as e,al as ie,av as ss,ao as as,a0 as te,aq as D,an as _e,ak as ts,ar as ls,aU as Te,ad as W,bU as ve,a_ as De,a9 as we,c4 as rs,cg as Re,a8 as Ie,cw as ns,at as os,a2 as is,aE as $e,a1 as ue}from"./vendor-react-Bs5V2qFE.js";import{D as le,a as re,b as ne,c as oe,d as Ce,n as Be,s as Le,v as be,L as g,I as N,w as K,B as n,T as Ge,j as ge,t as u,p as B,C as T,O as L,o as Oe,q as cs,r as ds,S as Q,e as ee,g as se,h as ae,i as J,k as Fe,x as Pe}from"./index-DLxf9dwO.js";import{S as ms}from"./SendMailDialog-DwU-GlVi.js";import"./vendor-radix-BQCqNqg0.js";import"./vendor-pdf-DtmgLs_2.js";
const JBC_VAULT_BACKUPS = {
  "JAI BHAVANI CARGO - LABOUR LICENSE": "/assets/docs/jai_bhavani_cargo_labour_license.pdf",
  "BANK VERIFIED LETTER": "/assets/docs/hdfc_bank_verified_letter_t8xpih3tbq.jpeg",
  "PASSPORT SIZE PHOTO": "/assets/docs/vinod_kumar_rathod_passport_size_photo_zbm84u58dk.jpeg",
  "UDYAM-TS-20-0193891": "/assets/docs/jai_bhavani_cargo_udhyam_annexure_70zsj9y030.pdf",
  "JAI BHAVANI CARGO - Cancelled Cheque": "/assets/docs/jai_bhavani_cargo_hdfc_cancelled_cheque_3gfuewnsk7.jpeg",
  "ADHAAR": "/assets/docs/vinod_kumar_rathod_adhaar_b9x1626gke.pdf",
  "HEAVY DRIVING LICENSE": "/assets/docs/img_2729_yzytzryqf3.JPG.jpeg",
  "PAN": "/assets/docs/vinod_kumar_rathod_pan_card_y1fu4ohh15.jpeg",
  "GST": "/assets/docs/jai_bhavani_cargo_gst_xt9lamef1x.pdf",
  "JAI BHAVANI CARGO - ALL TRUCK REGISTRATION CERTIFICATES": "/assets/docs/img_1909_7e4rqy1gf2_0arqs6jdqq.jpg"
};
function xs({isOpen:fe,onClose:G,companyInfo:d={},document:m=null,allDocuments:R=[],selectedDocIds:V=null}){const[a,z]=o.useState(""),[i,H]=o.useState(""),[v,Ne]=o.useState(""),[S,ce]=o.useState(!1),[A,q]=o.useState(m?"single":"dossier");es.useEffect(()=>{q(m?"single":"dossier")},[m,fe]);const I=o.useMemo(()=>Array.isArray(R)?V?R.filter(x=>V.has(x.id)):R:[],[R,V]),Y=o.useMemo(()=>{if(!m)return"";const x=a.trim()||"Sir / Madam",f=d.company_name||"JAI BHAVANI CARGO",l=d.company_gstin||"36DPXPR9171A1Z8";let b=`🏢 *${f} - OFFICIAL DOCUMENT SHARE*
...
e.jsxs("div",{className:"p-3.5 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-3",children:[
  ...
  let curUrl = activeTruck ? activeTruck.url : ((previewSide==="back"&&c.back_file_url)?c.back_file_url:c.file_url); if ((!curUrl || curUrl.includes("ze8jw9s6n72jtxn") || c.title==="JAI BHAVANI CARGO - LABOUR LICENSE") && typeof JBC_VAULT_BACKUPS!=="undefined" && JBC_VAULT_BACKUPS[c.title]) { curUrl = JBC_VAULT_BACKUPS[c.title]; } else if (!curUrl && typeof JBC_VAULT_BACKUPS!=="undefined" && JBC_VAULT_BACKUPS[c.title]) { curUrl = JBC_VAULT_BACKUPS[c.title]; }
  ...
  return e.jsxs(e.Fragment,{children:[
    ...
    e.jsxs(ge,{className:"shrink-0 pt-2 sm:pt-3 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-3",children:[
      ...
      e.jsxs("div",{className:"flex items-center gap-1.5 sm:gap-2 flex-wrap justify-end",children:[e.jsxs("label",{className:"border-slate-700 bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white text-xs font-semibold rounded-xl px-2.5 py-1.5 border flex items-center gap-1.5 cursor-pointer shadow-sm transition-colors",title:"Upload new version of this document",children:[e.jsx("input",{type:"file",className:"hidden",onChange:async ev=>{const f=ev.target.files?.[0];if(f){try{u.loading("Uploading replacement file...");const fd=new FormData();fd.append("file",f);fd.append("document_name",c.title);fd.append("document_type",c.sub_category||"Other");fd.append("expiry_date","2099-12-31 00:00:00.000Z");const r=await B.collection("truck_documents").create(fd,{$autoCancel:!1});const newUrl=B.files.getURL(r,r.file);const curDocs=(p.company_docs_json?JSON.parse(p.company_docs_json):[]).map(d=>(d.id===c.id||d.title===c.title)?{...d,file_url:newUrl,file_name:f.name,file_size:f.size,updated_at:new Date().toISOString()}:d);await B.collection("company_settings").update(a.id,{company_docs_json:JSON.stringify(curDocs)},{$autoCancel:!1});z(prev=>({...prev,company_docs_json:JSON.stringify(curDocs)}));c.file_url=newUrl;u.success("Document updated successfully!");}catch(err){u.error("Upload failed: "+(err.message||err));}}}}),e.jsx("span",{children:"🔄"}),"Replace File"]}),
