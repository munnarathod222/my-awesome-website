...
const InlineKmInput = ({ log, className, onUpdate }) => {
  ...
  const handleBlur = async () => {
    ...
    try {
      const payload = { kms: updatedVal };
      if (log.pod_status === 'Verified') {
        payload.pod_status = 'Uploaded';
      }
      await pb.collection('trip_logs').update(log.id, payload, { $autoCancel: false });
      toast.success(`KMs updated for Trip ${log.trip_id}`);
      if (onUpdate) onUpdate(updatedVal);
    } catch (err) {
      console.error('Failed to update kms:', err);
      if (err.data?.data?.pod_status || (err.message && err.message.includes('pod_status'))) {
        try {
          await pb.collection('trip_logs').update(log.id, { kms: updatedVal, pod_status: 'Uploaded' }, { $autoCancel: false });
          toast.success(`KMs updated for Trip ${log.trip_id}`);
          if (onUpdate) onUpdate(updatedVal);
          return;
        } catch (retryErr) {
          console.error('Retry with sanitized pod_status failed:', retryErr);
        }
      }
      toast.error(err.message || 'Failed to update KMs');
      setVal(log.kms || '');
    }
  };
...
const TripLogsPage = () => {
  ...
  const executeDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const targets = deleteDialogData.map(t => ({ id: t.id, trip_id: t.trip_id }));
      const deleteIds = targets.flatMap(t => [t.id, t.trip_id].filter(Boolean));
              ...
              <div className="md:hidden space-y-4 px-1 pb-4">
                {filteredLogs.length === 0 ? (
                  ...
                  paginatedLogs.map(log => (
                    ...
                    >
                      {/* Row 1: Trip ID, Date & Status */}
                      <div className="flex items-center justify-between gap-2 border-b border-border/40 pb-2">
                        <div className="flex items-center gap-2 flex-wrap min-w-0 flex-1">
                          <span className="font-mono font-bold text-xs text-primary bg-primary/10 border border-primary/25 px-2 py-0.5 rounded-lg shadow-sm whitespace-nowrap shrink-0">
                            {log.trip_id || 'N/A'}
                          </span>
                          <span className="inline-flex items-center gap-1.5 text-xs font-black text-sky-300 bg-sky-500/15 border border-sky-500/30 px-2.5 py-0.5 rounded-lg shadow-sm whitespace-nowrap shrink-0">
                            ...
                            {formatTripDate(log.date || log.created || log.trip_date)}
                          </span>
                        </div>
