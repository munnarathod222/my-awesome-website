...
import { ORR_INTERCHANGES, ORR_VEHICLE_CATEGORIES, calculateOrrToll } from '../src/data/orrTollRates.js';
...
export default function TripOverviewCalculator() {
  ...
  const [orrOrigin, setOrrOrigin] = useState('1'); // 1: Kokapet
  const [orrDestination, setOrrDestination] = useState('16'); // 16: Shamshabad
  const [orrVehicle, setOrrVehicle] = useState('bus_2axle');
  const [orrTripType, setOrrTripType] = useState('single'); // 'single' | 'return24h'
  const [orrAppliedAnimation, setOrrAppliedAnimation] = useState(false);
  ...
  const orrResult = useMemo(() => {
    return calculateOrrToll(orrOrigin, orrDestination, orrVehicle, orrTripType);
  }, [orrOrigin, orrDestination, orrVehicle, orrTripType]);

  const handleApplyOrrToll = () => {
    if (!orrResult || !orrResult.success) return;
    setTolls([orrResult.selectedFare]);
    setOrrAppliedAnimation(true);
    setTimeout(() => setOrrAppliedAnimation(false), 2000);
    const tripLabel = orrResult.tripType === 'return24h' ? '2-Way (24h Return)' : '1-Way Single';
    toast.success(`Applied ORR toll ₹${orrResult.selectedFare} (${orrResult.origin?.name} ➔ ${orrResult.destination?.name}, ${tripLabel})`);
  };
              ...
              <Card className="border-border shadow-sm bg-card">
                ...
                <CardContent className="space-y-6 pt-5">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="p-3.5 bg-slate-950/70 border border-border/80 rounded-xl space-y-3 shadow-inner">
                      <div className="flex items-center justify-between border-b border-border/40 pb-2">
                        ...
                        <span className="text-[10px] text-slate-400 font-medium">2024-25 Rate Matrix</span>
                      </div>

                      {/* Origin & Destination Selectors with Swap */}
                      <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 items-center">
                        <div className="sm:col-span-2 space-y-1">
                          <label className="text-[10px] font-bold text-slate-400">Origin Interchange</label>
                          <select
                            value={orrOrigin}
                        ...
                        <div className="sm:col-span-2 space-y-1">
                          <label className="text-[10px] font-bold text-slate-400">Destination Interchange</label>
                          <select
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-slate-400">Vehicle Category</label>
                        <select
                          value={orrVehicle}
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-center text-[10px] font-bold text-slate-400">
                          ...
                          {orrTripType === 'return24h' && (
                            <span className="text-emerald-400 font-semibold">24h Return Pass (1.5× Rate)</span>
                          )}
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <button
                            ...
                            className={`py-1.5 px-2.5 rounded-lg text-xs font-bold transition flex items-center justify-center gap-1.5 ${
                              orrTripType === 'single'
                                ? 'bg-primary text-primary-foreground shadow-sm'
                                : 'bg-muted/30 border border-border/50 text-slate-400 hover:text-white'
                            }`}
                          >
                            <span>1-Way Single</span>
                            <span className="text-[10px] opacity-80 font-mono">₹{orrResult?.singleFare || 0}</span>
                          </button>
                          <button
                            type="button"
                          ...
                          >
                            ...
                            <span className="text-[10px] opacity-80 font-mono">₹{orrResult?.returnFare24h || 0}</span>
                          </button>
                        </div>
                      </div>

                      {/* Result Display & Apply Button */}
                      <div className="pt-2.5 border-t border-border/40 flex items-center justify-between gap-3">
                        <div>
